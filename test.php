    ...
    function showAddOnModal(productName, price, id) {
      ...
      $.ajax({
        ...
        success: function(response){
            ...
            let freeAddOnHTML = `<p style="font-weight: 900; font-size: large;"> Free </p> <table class="table table-striped"> 
            <tbody>`;
            let extraAddOnHTML = `<p style="font-weight: 900; font-size: large;"> Extra </p> <table class="table table-striped"> 
            <tbody>`;
            let free=0;
            let extra=0;
            data.forEach(function(item) {
                if(item.category === "free"){
                    free=1;
                    freeAddOnHTML+= `<tr>
                    <td><label for="addon1"> ${item.name} </label></td>
                        <td><input type="radio" class="form-check-input addon-checkbox" id="addon1" value="${item.name}" data-price="0.00" name="addon"> </td>
                    </tr>`
                }
                else if(item.category === "extra"){
                    extra=1;
                    extraAddOnHTML+= `<tr>
                    <td><label for="addon1"> ${item.name} - ($${item.price}) </label></td>
                        <td><input type="checkbox" class="form-check-input addon-checkbox" id="addon1" value="${item.name}" data-price="${item.price}" name="addon"> </td>
                    </tr>`
                }
            ...
            extraAddOnHTML+=` </tbody>
            </table>`;
            TotalAddOnHTML = "";
            if(free==1){
                TotalAddOnHTML+=freeAddOnHTML;
            }
            if(extra==1){
                TotalAddOnHTML+=extraAddOnHTML;
            }

            document.getElementById('addonOptions').innerHTML = TotalAddOnHTML;
